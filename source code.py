...
class GymHelper(QMainWindow):
    ...
    def Page1UI(self, page):
        ...
        page.Checkboxes = []
        self.BulkBox = QCheckBox("Набрать мышечную массу")
        self.SkinnyBox = QCheckBox("Похудеть")
        self.HealthBox = QCheckBox("Укрепить здоровье")
        self.SelectedGoals = QLabel("Выбранные цели:")
        ...
        self.BulkBox.stateChanged.connect(self.UpdateGoals)
        self.SkinnyBox.stateChanged.connect(self.UpdateGoals)
        self.HealthBox.stateChanged.connect(self.UpdateGoals)
    ...
    def UpdateGoals(self): # Обновление информации о целях в базе данных
        ...
        self.SelectedGoals.setText("Выбранные цели: " + ", ".join(Selected))
...
class BodyFatDialog(QDialog):
    def __init__(self, database):
        ...
        self.BodyFat = round(self.Db['Weight'] / (self.Db['Height'] ** 2), 2)
        self.BodyFatLabel = QLabel('Процент жира:')
        self.BodyFatLabel2 = QLabel(str(self.BodyFat))
        Layout.addWidget(self.BodyFatLabel)
        Layout.addWidget(self.BodyFatLabel2)
        age = self.Db['Age']
        gender = self.Db['Sex']
        if 16 <= age <= 25:
            if gender == 'Мужской':
